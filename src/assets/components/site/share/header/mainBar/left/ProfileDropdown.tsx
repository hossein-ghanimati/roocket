  ...
  return (
    <Dropdown dir="rtl">
      ...
      <DropdownMenu>
        <DropdownItem onClick={() => overlaySetting?.hideOverlay()}>
          <div className="flex items-center justify-start m-2 mb-3 pb-3 border-b border-gray-300 dark:border-gray-500 gap-4 ">
            ...
            <div className="flex flex-col">
              <Link onClick={() => overlaySetting?.hideOverlay()} to="/panel" className="font-yekan text-lg">
                حسین غنیمتی
              </Link>
              <Link onClick={() => overlaySetting?.hideOverlay()}
                to="/panel"
        ...
        <DropdownItem onClick={() => overlaySetting?.hideOverlay()}>
          <Link onClick={() => overlaySetting?.hideOverlay()} to="/panel/courses" className="flex items-center gap-3 text-base">
            <BsCameraVideo className="size-5"/>
            <span>دوره ها</span>
          </Link>
        </DropdownItem>
        <DropdownItem onClick={() => overlaySetting?.hideOverlay()}>
          <Link onClick={() => overlaySetting?.hideOverlay()} to="/panel/tickets" className="flex items-center gap-3 text-base">
