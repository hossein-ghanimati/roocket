...
const MobileFilterOptions = () => {
  ...
  const [onlyBought, setOnlyBought] = useState(coursesFilterSetting?.isOnlyBought || false)
  ...
  return (
    <div className={`${filterMenuSetting?.isMenuOpen ? "" : "invisible opacity-0"} md:hidden fixed inset-0 z-20 flex size-full flex-col bg-white dark:bg-gray-900 transition-all`}>
      ...
      >
        <FilterOption checked={onlyFree} onChange={e => setOnlyFree(e.target.checked)} title="فقط دوره های رایگان"/>
        <FilterOption checked={onlyBought} onChange={e => setOnlyBought(e.target.checked)} title="دوره های خریداری شده"/>
      </div>
      <div
        className="mt-5 flex h-full items-end px-5 pb-8"
      >
        <button onClick={() => coursesFilterSetting?.acceptChange(onlyFree, onlyBought)} className="w-full text-white bg-green-500 px-4 h-12 leading-7 cursor-pointer select-none rounded-full">
          اعمال فیلتر
        </button>
      </div>
    </div>
